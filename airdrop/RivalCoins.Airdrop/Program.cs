...
var wallet = new RivalCoins.Sdk.Wallet("https://localhost:8001", AirdropAccountSeed, "https://rivalcoins.money");
...
async Task AirdropAsync(IList<(string Destination, stellar_dotnet_sdk.AssetTypeCreditAlphaNum Asset)> airdropInfos)
{
    ...
    if (airdropInfos.Any())
    {
        using var server = new Server(wallet.NetworkUrl);
        var airdropAccount = await server.Accounts.Account(wallet.Account.Info.AccountId);
        ...
        finalTx.Sign(wallet.Account.Signer, wallet.NetworkInfo);
